...
public class ImageUtils {
  ...
  public static void writePixels(
      WritableImage wImage, int x, int y, int width, int height, int[] pixels) { // {{{
  ...
  /**
   * 渡したpixel配列を書き込んだ新しい画像を返す。 座標はx=0, y=0から書き込む。
   *
   * @param wImage 書き込む対象
   * @param width ピクセル横幅
   * @param height ピクセル縦幅
  ...
  /**
   * 画像ファイルのリストから連結した一つの画像を生成し返す。 連結する画像の幅は、それぞれ渡されたリストの先頭の要素を基準とする。
   *
  ...
  public static Image joinImageFiles(List<File> imageList) { // {{{
    ...
    // 連結画像書き込み用のイメージの生成
    int newImageHeight = filteredImageList.stream().mapToInt(img -> (int) img.getHeight()).sum();
